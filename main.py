...
frame_direita = Frame(janela,bg=cor1,padx=5)
frame_direita.grid(row=0,column=1)

frame_baixo = Frame(janela,bg='white')
...
def escala(valor):
    ...
    #alterando  acor do fundo da tela
    tela['bg'] = hexadecimal
...
l_rgb = Label(frame_baixo,text='CODIGO RGB',bg=cor1,font=('Ivy',10,'bold'))
...
#botão copiar
b_copiar = Button(frame_baixo,command=onclicar,text='Copiar  a cor',bg=cor1,font=('Ivy',10,'bold'),relief=RAISED,overrelief=RIDGE)
b_copiar.grid(row=0,column=2,padx=5)

#app nome----------
l_app_nome = Label(frame_baixo,text='Seletor de Cores',bg=cor1,font=('Ivy',15,'bold'))
l_app_nome.grid(row=0,column=3,padx=35)
